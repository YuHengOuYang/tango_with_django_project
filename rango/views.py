...
# that will be passed to the template engine.
    ...
    #context_dict['visits'] =request.session['visits']#又不用强制转换了
...
def add_category(request):
    form = CategoryForm()
    # A HTTP POST?
    if request.method == 'POST':#表示用户提交了表单
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            profile = profile_form.save(commit=False)#这里很重要，我们先不保存，因为有外键，把外键加上再说
            profile.user = user
    ...
    return render(request,
        ...
        context = {'user_form': user_form,
                    ...
                    'registered': registered})#为什么还会有registered字段？这个表单要经历一次get一次post的！
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.is_active:#检索active字段
                # If the account is valid and active, we can log the user in.
                # We'll send the user back to the homepage.
                login(request, user)#但是我还是不知道为什么登录之后所有的页面都可以用user了
...
def get_server_side_cookie(request, cookie, default_val=None):
    val = request.session.get(cookie)#这里的cookie是要找的字符串
    if not val:
        val = default_val
    return val

def visitor_cookie_handler(request):#用session，信息全部存在server端，用不到response了
    ...
    #response.set_cookie('visits', visits)#注意没有返回http response，所以严格不能算一个view